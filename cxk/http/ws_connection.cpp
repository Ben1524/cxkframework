...
std::pair<HttpResult::ptr, WSConnection::ptr> WSConnection::Create(const std::string& url, 
    ...
    if(!uri){
        return std::make_pair(std::make_shared<HttpResult>((int)HttpResult::Error::INVALID_URL, 
            nullptr, "invaliid url:" + url), nullptr);
    }
...
std::pair<HttpResult::ptr, WSConnection::ptr> WSConnection::Create(Uri::ptr uri, uint64_t timeout_ms, 
    ...
    if(!addr){
        return std::make_pair(std::make_shared<HttpResult>((int)HttpResult::Error::INVALID_HOST, 
            nullptr, "invalied host: " + uri->getHost()), nullptr);
    }
    ...
    if(!sock->connect(addr)){
        return std::make_pair(std::make_shared<HttpResult>((int)HttpResult::Error::CONNECT_FALT, nullptr, 
    ...
    req->setHeader("Sec-webSocket-Key", cxk::base64encode(random_string(16)));
    if(!has_host){
    ...
    if(!rsp){
        return std::make_pair(std::make_shared<HttpResult>((int)HttpResult::Error::TIMEOUT, 
            ...
            + std::to_string(timeout_ms)), nullptr);
    }

    if(rsp->getStatus() != HttpStatus::SWITCHING_PROTOCOLS){
        return std::make_pair(std::make_shared<HttpResult>(50, rsp, "not websocket server " + addr->toString()), nullptr);
    }
