...
int SQLite3Stmt::bind(int idx, const char* value, int len, Type type){
    return sqlite3_bind_text(m_stmt, idx, value, len, type == COPY ? SQLITE_TRANSIENT : SQLITE_STATIC);
}
...
SQLite3Data::SQLite3Data(std::shared_ptr<SQLite3Stmt> stmt) : m_stmt(stmt) {
...
SQLite3Transaction::SQLite3Transaction(SQLite3::ptr db, bool auto_commit, Type type) : m_db(db), m_type(type), 
    m_status(0), m_autoCommit(auto_commit) {
...
SQLite3Transaction::~SQLite3Transaction(){
    if(m_status == 1){
        ...
        if(m_status == 1){
            CXK_LOG_ERROR(g_logger) << m_db << " auto commit error"; 
        }
    }
}
