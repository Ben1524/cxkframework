...
AsyncSocketStream::AsyncSocketStream(Socket::ptr sock, bool owner) : SocketStream(sock, owner), m_waitSem(2),
    m_sn(0), m_autoConnect(false), m_iomanager(nullptr){
...
bool AsyncSocketStream::start(){
    if(!m_iomanager){
        m_iomanager = cxk::IOManager::GetThis();
    }
...
void AsyncSocketStream::doRead(){
    ...
    CXK_LOG_DEBUG(g_logger) << "doRead out" << this;
    innerClose();
    m_waitSem.notify();
...
void AsyncSocketStream::doWrite(){
    ...
    CXK_LOG_DEBUG(g_logger) << "doWrite out" << this;
    {
        RWMutexType::WriteLock lock(m_queueMutex);
        m_queue.clear();
    }
...
void AsyncSocketStream::Ctx::doRsp(){   
    ...
    // 确保有一个线程可以将scheduler置为nullptr
    if(!cxk::Atomic::compareAndSwapBool(scheduler, scd, (Scheduler*)nullptr)){
        return;
    }
...
AsyncSocketStream::ptr AsyncSocketStreamManager::get(){
    ...
    for(uint32_t i = 0; i < m_size; ++i){
        auto idx = cxk::Atomic::addFetch(m_idx, 1);
...
void AsyncSocketStreamManager::setConnectCb(const connect_callback& cb){
    m_connectCb = cb;
    RWMutexType::WriteLock lock(m_mutex);
    for(auto& i : m_datas){
        i->setConnectCb(cb);
...
void AsyncSocketStreamManager::setDisconnectCb(const disconnect_callback& cb){
    m_disconnectCb = cb;
    RWMutexType::WriteLock lock(m_mutex);
    for(auto& i : m_datas){
        i->setDisconnectCb(cb);
    }
}
