...
public:
    ...
    /// @post   m_position += sizeof(value)， 如果m_position > m_size 则 m_size = m_position        
    void writeFint8(int8_t value);

    /// @brief  写入固定长度uint8_t类型的数据
    /// @post   m_position += sizeof(value)， 如果m_position > m_size 则 m_size = m_position  
    void writeFuint8_t(uint8_t value);
    ...
    /// @post   m_position += sizeof(value)， 如果m_position > m_size 则 m_size = m_position 
    void writeFuint16_t(uint16_t value);
    ...
    /// @post   m_position += sizeof(value)， 如果m_position > m_size 则 m_size = m_position 
    void writeFuint32_t(uint32_t value);
    ...
    /// @post   m_position += sizeof(value)， 如果m_position > m_size 则 m_size = m_position 
    void writeFuint64_t(uint64_t value);

    /// @brief  写入有符号Varint32类型的数据
    /// @post   m_postion += 实际占用内存(1 ~ 5), 如果m_postion > m_size 则 m_size = m_position        
    ...
    /// @post   m_postion += 实际占用内存(1 ~ 10), 如果m_postion > m_size 则 m_size = m_position 
    ...
    /// @brief  写入float类型的数据
    /// @post   m_postion += sizeof(value), 如果m_position > m_size 则 m_size = m_position
    ...
    /// @post   m_position += 2 + value.size()，如果m_position > m_size 则 m_size = m_position
    void writeStringF16(const std::string& str);

    /// @brief  写入std::string类型的数据，用uint32_t作为长度类型
    /// @post   m_position += 4 + value.size()，如果m_position > m_size 则 m_size = m_position
    void writeStringF32(const std::string& str);


    /// @brief  写入std::string类型的数据，用uint64_t作为长度类型
    /// @post   m_position += 8 + value.size()，如果m_position > m_size 则 m_size = m_position
    void writeStringF64(const std::string& str);


    /// @brief  写入std::string类型的数据，用Varint64作为长度类型
    /// @post   m_position += Varint64长度 + value.size()，如果m_position > m_size 则 m_size = m_position
    void writeStringVint(const std::string& str);

    /// @brief  写入std::string类型的数据,无长度，如果m_position > m_size 则 m_size = m_position
    /// @post   m_position += 8 + value.size()
    void writeStringWithoutLength(const std::string& str);
    ...
    uint64_t readFuint64_t();

    /// @brief      读取Varint32_t类型的数据
    /// @pre        getReadSize() >= 有符号Varint32实际内存大小
    /// @post       m_position += 有符号Varint32的实际占用内存
    /// @exception 如果getReadSize() < 有符号Varint32实际占用内存 抛出std::out_of_range
    int32_t readInt32();
    ...
    /// @brief      读取Varint64_t类型的数据
    /// @pre        getReadSize() >= 有符号Varint64实际内存大小
    /// @post       m_position += 有符号Varint64的实际占用内存
    /// @exception 如果getReadSize() < 有符号Varint64实际占用内存 抛出std::out_of_range
    int64_t readInt64();

    /// @brief      读取Varuint64_t类型的数据
    /// @pre        getReadSize() >= 有符号Vaurint64实际内存大小
    /// @post       m_position += 有符号Varuint64的实际占用内存
    /// @exception 如果getReadSize() < 有符号Varuint64实际占用内存 抛出std::out_of_range
    uint64_t readUint64();
    ...
    /// @exception 如果getReadSize() < sizeof(float) 抛出std::out_of_range
    float readFloat();

    /// @brief      读取double类型的数据
    /// @pre        getReadSize() >= sizeof(double)
    /// @post       m_position += sizeof(double)
    /// @exception 如果getReadSize() < sizeof(double) 抛出std::out_of_range
    double readDouble();
    ...
    /// @exception  getReadSize() < sizeof(uint16_t) + size 抛出 std::out_of_range
    ...
    /// @brief      读取std::string类型的数据，用int64_t作为长度
    /// @pre        getReadSize() >= sizeof(int64_t) + size
    /// @post       m_position += sizeof(int64_t) + size
    /// @exception  getReadSize() < sizeof(int64_t) + size 抛出 std::out_of_range
    std::string readStringF64();


    /// @brief      读取std::string类型的数据，用Varint64作为长度
    /// @pre        getReadSize() >= Varint64的实际长度 + size
    /// @post       m_position += Varint64的实际长度 + size
    /// @exception  getReadSize() < Varint64实际长度 + size 抛出 std::out_of_range
    std::string readStringVint();
    
    /// @brief  清空ByteArray
    /// @post   m_positin = 0, m_size = 0;
    void clear();
    ...
    /// @param buf  内存缓存指针
    /// @param size 数据大小
    /// @post       m_position += size， 如果m_position > m_size， 则m_size = m_position
    ...
    /// @post       如果m_position > m_size, 则m_size = m_position
    ...
    /// @brief  讲ByteArray里面的数据[m_position, m_size]转成std::string
    std::string toString();

    /// @brief  讲ByteArray里面的数据[m_position, m_size]转成16进制的std::string
    ...
    /// @return             返回实际数据的长度
    uint64_t getReadBuffers(std::vector<iovec>& buffers, size_t len, uint64_t position) const;
...
private:
    ...
    /// @brief  扩容ByteArray，使其可以容纳size歌数据(如果原本可以容纳就不扩容)
    void addCapacity(size_t size);

    /// @brief  获取当前的可写入容量
    size_t getCapacity() const {return m_capacity - m_position;}
private:
    size_t m_baseSize;  // 每一个Node大概有多大 
    size_t m_position;  // 当前的位置
