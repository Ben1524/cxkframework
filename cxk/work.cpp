...
static cxk::ConfigVar<std::map<std::string, std::map<std::string, std::string>>>::ptr g_worker_config
    = cxk::Config::Lookup("workers", std::map<std::string, std::map<std::string, std::string>>(), "worker config");




WorkerGroup::WorkerGroup(uint32_t batch_size, cxk::Scheduler* s)
...
bool WorkerManager::init(){
    ...
    for(auto& i : workers){
        ...
        int32_t thread_num = cxk::GetParamValue(i.second, "thread_num", 1);
        int32_t worker_num = cxk::GetParamValue(i.second, "worker_num", 1);
