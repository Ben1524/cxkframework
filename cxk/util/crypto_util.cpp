...
int32_t CryptoUtil::Crypto(const EVP_CIPHER* cipher, bool enc,
    ...
    int tmp_len = 0;
    bool has_error = false;
    EVP_CIPHER_CTX* ctx = EVP_CIPHER_CTX_new(); // 使用new分配ctx
    if(ctx == nullptr) {
    return -1; // 分配失败处理
    }

    do {
    if(1 != EVP_CipherInit_ex(ctx, cipher, nullptr, static_cast<const unsigned char*>(key),
            static_cast<const unsigned char*>(iv), enc ? 1 : 0)) {
    has_error = true;
    break;
    }

    if(1 != EVP_CipherUpdate(ctx, static_cast<unsigned char*>(out), &tmp_len,
            static_cast<const unsigned char*>(in), in_len)) {
    has_error = true;
    break;
    }
    *out_len = tmp_len;

    if(1 != EVP_CipherFinal_ex(ctx, static_cast<unsigned char*>(out) + tmp_len, &tmp_len)) {
    has_error = true;
    break;
    }
    *out_len += tmp_len;
